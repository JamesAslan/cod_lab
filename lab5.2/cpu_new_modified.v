...
module mycpu(
    ...
    (*mark_debug = "true"*)output inst_ram_wr,
    (*mark_debug = "true"*)output [1:0] inst_ram_size,   
	(*mark_debug = "true"*)output [31:0] inst_sram_addr,
	(*mark_debug = "true"*)input  [31:0] inst_sram_rdata,
	(*mark_debug = "true"*)output [31:0] inst_sram_wdata,
	//output [3:0] inst_sram_wen,
    (*mark_debug = "true"*)input  inst_sram_addr_ok,  
    (*mark_debug = "true"*)input  inst_sram_data_ok,  
	
	(*mark_debug = "true"*)output data_sram_en,  
	...
	(*mark_debug = "true"*)output [3:0] data_sram_wen,
	...
	(*mark_debug = "true"*)assign inst_sram_en = resetn &&(inst_state[0] /*|| (irp_counter3 == 2'b11)*/) && resetn &&  !branch_valid && ((!(syscall_info2 || syscall_info) && !break_info2 && !irp_ip1_2 && !irp_ip0_2  && !irp_inst2 && !irp_time2 && irp_support2 && !irp_overflow && !irp_address_lw && !irp_address_sw) || pipe1_allowin);
    //&& !irp_ip2_2 && !irp_ip3_2 && !irp_ip4_2 && !irp_ip5_2 && !irp_ip6_2 && !irp_ip7_2 

	(*mark_debug = "true"*)assign data_sram_en = (MemRead | MemWrite) && data_state[0] && resetn;
    assign data_sram_wr = MemWrite;
	(*mark_debug = "true"*)assign data_sram_wen = /*{MemWrite,MemWrite,MemWrite,MemWrite}& */Write_strb;
	...
	reg [2:0] inst_irp_treat;
	always @(posedge clk)
	begin
		if(rst)
		begin
			inst_irp_treat <= 3'b000;
		end
    ...
    (*mark_debug = "true"*)reg [2:0] inst_state;
    always @(posedge clk)
	begin
		if(rst)
		begin
    ...
    (*mark_debug = "true"*)reg [2:0] data_state;
    always @(posedge clk)
	begin
		if(rst)
		begin
	...
	////////////////////////////////////////////////////////////////////
	//  pipe1
	(*mark_debug = "true"*)reg [31:0] pipe1_data;
	(*mark_debug = "true"*)reg [31:0] pipe1_PC;
	(*mark_debug = "true"*)reg pipe1_valid;
	(*mark_debug = "true"*)wire pipe1_allowin;
	(*mark_debug = "true"*)wire pipe1_readyout;
	wire pipe1_outvalid;
	assign pipe1_allowin = !pipe1_valid || (pipe2_allowin && pipe1_readyout);
	assign pipe1_readyout = /*inst_sram_data_ok;*/(inst_state == 3'b000);
	...
	(*mark_debug = "true"*)reg [1:0]counter_choke;
	always@(posedge clk)
	begin
	...
	//  pipe2
	reg syscall_info2;
	(*mark_debug = "true"*)reg [31:0] pipe2_PC;
	(*mark_debug = "true"*)reg pipe2_valid;
	wire pipe2_allowin;
	wire pipe2_readyout;
	wire pipe2_outvalid;
	assign pipe2_allowin = !pipe2_valid || (pipe3_allowin && pipe2_readyout);
	assign pipe2_readyout = !counter_lw_choke && (!counter_div_choke)&& (!(op_div_reg == 1'b1 && op_div_last_reg ==1'b0) || syscall_info3) && (!(op_mul_reg == 1'b1 && op_mul_last_reg ==1'b0)|| syscall_info3) && counter_branch;
	...
	begin
		...
		begin
			...
			memRead2 <= memRead;
			memWrite2 <= memWrite;
			branch_or_not_reg <= branch_or_not_input;
			mthi_reg <= mthi;
			mtlo_reg <= mtlo;
		...
		else if((pipe1_outvalid && pipe2_allowin) && counter_branch)
		begin
	...
	(*mark_debug = "true"*)wire [31:0] pipe2_data_in;
	...
	(*mark_debug = "true"*)reg [31:0] pipe2_data;
	...
	assign forward_pipe3_r2 = ((raddr2 == waddr_dst2) && (raddr2 != 5'b0)) && ((pipe2_data[31:26] != `LW) && (pipe2_data[31:26] != `LB) && (pipe2_data[31:26] != `LBU) && (pipe2_data[31:26] != `LH) && (pipe2_data[31:26] != `LHU) && (pipe2_data[31:26] != `LWL) && (pipe2_data[31:26] != `LWR)) && (cp0_wen || (pipe2_data_in[31:26] == 6'b0) || (pipe2_data_in[31:26] == `BEQ) || (pipe2_data_in[31:26] == `BNE) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZ) || (pipe2_data_in[31:26] == `BLEZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `BGTZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZ) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZAL) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZAL) || (pipe2_data_in[31:26] == `SW) || (pipe2_data_in[31:26] == `SB) || (pipe2_data_in[31:26] == `SH) || (pipe2_data_in[31:26] == `SWL) || (pipe2_data_in[31:26] == `SWR)) && reg_write2 ;

	wire forward_pipe4_r1;
	wire forward_pipe4_r2;
	assign forward_pipe4_r1 = ((raddr1 == waddr_dst3) && (raddr1 != 5'b0)) && reg_write3;
	assign forward_pipe4_r2 = ((raddr2 == waddr_dst3) && (raddr2 != 5'b0)) && (cp0_wen || (pipe2_data_in[31:26] == 6'b0) || (pipe2_data_in[31:26] == `BEQ) || (pipe2_data_in[31:26] == `BNE) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZ) || (pipe2_data_in[31:26] == `BLEZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `BGTZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZ) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZAL) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZAL) || (pipe2_data_in[31:26] == `SW) || (pipe2_data_in[31:26] == `SB) || (pipe2_data_in[31:26] == `SH) || (pipe2_data_in[31:26] == `SWL) || (pipe2_data_in[31:26] == `SWR)) && reg_write3;

	wire forward_wait5_r1;
	wire forward_wait5_r2;
	assign forward_wait5_r1 = ((raddr1 == waddr_dst4) && (raddr1 != 5'b0)) && reg_write4;
	assign forward_wait5_r2 = ((raddr2 == waddr_dst4) && (raddr2 != 5'b0)) && (cp0_wen || (pipe2_data_in[31:26] == 6'b0) || (pipe2_data_in[31:26] == `BEQ) || (pipe2_data_in[31:26] == `BNE) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZ) || (pipe2_data_in[31:26] == `BLEZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `BGTZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZ) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZAL) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZAL) || (pipe2_data_in[31:26] == `SW) || (pipe2_data_in[31:26] == `SB) || (pipe2_data_in[31:26] == `SH) || (pipe2_data_in[31:26] == `SWL) || (pipe2_data_in[31:26] == `SWR)) && reg_write4;
	...
	wire lw_choke_r1;
	wire lw_choke_r2;
	(*mark_debug = "true"*)reg lw_choke_r1_reg;
	(*mark_debug = "true"*)reg lw_choke_r2_reg;
	...
	assign lw_choke_r2 = ((raddr2 == waddr_dst2) && (raddr2 != 5'b0)) && (pipe2_data[31:26] == `LW || pipe2_data[31:26] == `LB || pipe2_data[31:26] == `LBU || pipe2_data[31:26] == `LH || pipe2_data[31:26] == `LHU || pipe2_data[31:26] == `LWL || pipe2_data[31:26] == `LWR) && (pipe2_data_in[31:26] == 6'b0 || (pipe2_data_in[31:26] == `BEQ) || (pipe2_data_in[31:26] == `BNE) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZ) || (pipe2_data_in[31:26] == `BLEZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `BGTZ && pipe2_data_in[20:16] == 5'b0) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZ) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BLTZAL) || (pipe2_data_in[31:26] == `REGIMM && pipe2_data_in[20:16] == `BGEZAL) || pipe2_data_in[31:26] == `SW || pipe2_data_in[31:26] == `SB || pipe2_data_in[31:26] == `SH || pipe2_data_in[31:26] == `SWL || pipe2_data_in[31:26] == `SWR) && reg_write2;
	...
	begin
		...
		else if (branch_or_not_input && pipe2_valid && pipe1_allowin && counter_branch && !counter_lw_choke) ////////////////?????????????????????!!!!!!!!!!!!!!!!!!!!
		begin
			PC_reg <= pipe1_PC + Shift_left2_reg +32'd4;
		end
		else if (branch_or_not_input && pipe2_valid && counter_branch &&!counter_lw_choke) ////////////////?????????????????????!!!!!!!!!!!!!!!!!!!!
		begin
			PC_reg <= pipe1_PC + Shift_left2_reg;
		end
		else if (jump_or_not && pipe2_valid && pipe1_allowin && counter_branch &&!counter_lw_choke)
		begin
			PC_reg <= jump_target +32'd4;
		end
		else if (jump_or_not && pipe2_valid && counter_branch &&!counter_lw_choke) ////////////////?????????????????????!!!!!!!!!!!!!!!!!!!!
		begin
			PC_reg <= jump_target;
		end
	...
	wire eq_or_not;
	assign beq = (pipe2_data[31:26] == `BEQ );
	assign bne = (pipe2_data[31:26] == `BNE );
	assign bgez = (pipe2_data[31:26] == `REGIMM && pipe2_data[20:16] == `BGEZ);
	assign blez = (pipe2_data[31:26] == `BLEZ && pipe2_data[20:16] == 5'b0);
	assign bgtz = (pipe2_data[31:26] == `BGTZ && pipe2_data[20:16] == 5'b0);
	assign bltz = (pipe2_data[31:26] == `REGIMM && pipe2_data[20:16] == `BLTZ);
	assign bltzal = (pipe2_data[31:26] == `REGIMM && pipe2_data[20:16] == `BLTZAL);
	assign bgezal = (pipe2_data[31:26] == `REGIMM && pipe2_data[20:16] == `BGEZAL);
	assign eq_or_not = (alu_A_input == alu_B_input); 
	assign neg_or_not = alu_A_input[31];
	assign pos_or_not = ((alu_A_input[31] == 1'b1) || (alu_A_input == 32'b0));
	(*mark_debug = "true"*)assign branch_or_not_input= branch_or_not && !branch_valid;
    (*mark_debug = "true"*)wire    branch_or_not;
   ...
   (*mark_debug = "true"*) wire branch_valid;
	...
	always@(posedge clk)
	begin
		if(rst || ((branch_choke == 2'd0) && pipe1_valid))
		begin
			counter_branch <= 2'b1;
		end
		else if(branch_choke)
		begin
			counter_branch <= 2'b0;
		end
		else 
	...
	wire j;
	wire jalr;
	assign jal 	= 	(pipe2_data[31:26] == `JAL );
	assign jr 	= 	(pipe2_data[31:26] == 6'b0 && pipe2_data[5:0] == `JR );
	assign j 	= 	(pipe2_data[31:26] == `J );
	assign jalr = 	(pipe2_data[31:26] == 6'b0 && pipe2_data[20:16] == 5'b0 && pipe2_data[5:0] == `JALR );
	assign jump_or_not = jal || jr || j || jalr;
	
	wire[31:0] jump_target;
	assign jump_target = 	({32{jal || j }} & {pipe2_PC[31:28],pipe2_data[25:0],2'b0}) |
							({32{jr || jalr}} & alu_A_input) ;
	
	//assign PC_change = (jump_or_not || branch_or_not_input ||eret ||syscall_info3) && pipe2_valid && (!counter_forwarding) && (!counter_lw_choke);
	...
	begin
		...
		else if(branch_or_not_input || jump_or_not)
		begin
