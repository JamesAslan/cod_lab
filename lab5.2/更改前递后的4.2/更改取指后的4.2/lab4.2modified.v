...
module mycpu_top(
	...
	reg	 irp_inst2;
	reg	 irp_inst3;
	assign irp_inst = (inst_sram_addr[1:0] != 2'b0) ? 1'b1 : 1'b0;
	...
	begin
		...
		else if (branch_or_not && pipe2_valid && pipe1_allowin && !counter_lw_choke) ////////////////?????????????????????!!!!!!!!!!!!!!!!!!!!
		begin
			PC_reg <= pipe1_PC + Shift_left2_reg +32'd4;
		end
		else if (branch_or_not && pipe2_valid &&!counter_lw_choke) ////////////////?????????????????????!!!!!!!!!!!!!!!!!!!!
		begin
			PC_reg <= pipe1_PC + Shift_left2_reg;
		end
		else if (jump_or_not && pipe2_valid && pipe1_allowin &&!counter_lw_choke)
		begin
			PC_reg <= jump_target +32'd4;
		end
		else if (jump_or_not && pipe2_valid &&!counter_lw_choke) ////////////////?????????????????????!!!!!!!!!!!!!!!!!!!!
		begin
			PC_reg <= jump_target;
		end
	...
	assign branch_A_input = (lw_choke_r1) ? rdata1_reg :
						  	(forward_pipe3_r1 ) ? choose_data3 :
						  	(forward_pipe4_r1 ) ? wdata :
						  	(forward_wait5_r1 ) ? ALU_out5 :
						  	rdata1_reg;

	assign branch_B_input = (lw_choke_r2 ) ? rdata2_reg :
	...
	assign PC_branchafter = pipe1_PC + Shift_left2;
	wire branch_or_not;
	wire eq_or_not;
